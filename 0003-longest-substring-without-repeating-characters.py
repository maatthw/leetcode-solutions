...
# Space Complexity : O(min(n, m)) -> Seen stores characters. There are only so many possible characters that exist.
    ...
    # O(1) would be the more "precise" answer given the constraints, but O(min(n, m)) is a more general answer that covers a wider range of character set constraint variations